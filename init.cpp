...
static struct _commands commands[] = {
  ...
  { "multi-thread",		&opt.multi_thread ,			cmd_boolean },
  ...
  { "thread-count",	&opt.thread_count ,	cmd_number_inf },
...
defaults (void)
{
  ...
  //  opt.restrict_files_os = 0 ;
#else
  //opt.restrict_files_os = 1 ;
#endif
...
/*
  ...
  if (env && *env)
	...
	{
	  fprintf (stderr,  "%s: WGETRC points to %s, which doesn't exist.\n"),
		   exec_name, env);
	  exit (1);
	}
      return strdup (env);
...
static enum parse_line parse_line (const char *, char **,
					   char **, int *);
 ...
 // while ((line = read_whole_line (fp)) != NULL)
	...
	case line_ok:
	  ...
	  if (!setval_internal (comind, com, val))
	    {
	      fprintf (stderr,  "%s: Error in %s at line %d.\n",
		       "exec_name", file, ln);
	      ++errcnt;
	    }
	  break;
	case line_syntax_error:
	  fprintf (stderr,  "%s: Syntax error in %s at line %d.\n",
		   "exec_name", file, ln);
	  ++errcnt;
	  break;
	case line_unknown_command:
	  fprintf (stderr,  "%s: Unknown command `%s' in %s at line %d.\n",
...
run_command (const char *opt)
{
  ...
  switch (parse_line (opt, &com, &val, &comind))
	...
	exit (2);
      ...
      break;
    default:
      fprintf (stderr,  "%s: Invalid --execute command `%s'\n",
	       "exec_name", opt);
      exit (2);
    }
}
...
static int decode_string (const char *, const struct decode_item *,
				  int, int *);
...
cmd_bytes (const char *com, const char *val, void *place)
{
  ...
  if (!parse_bytes_helper (val, &byte_value))
    {
      fprintf (stderr,  "%s: %s: Invalid byte value `%s'\n",
	       "exec_name", com, val);
      return 0;
    }
  *(long *)place = (long)byte_value;
  return 1;
}
...
cmd_time (const char *com, const char *val, void *place)
{
  ...
  if (val == end)
    {
    err:
      fprintf (stderr,  "%s: %s: Invalid time period `%s'\n",
	       "exec_name", com, val);
      return 0;
    }
...
cmd_cert_type (const char *com, const char *val, void *place)
{
	...
	int ok = decode_string (val, choices, countof (choices), (int*)place);
	  if (!ok)
...
#define VAL_IS(string_literal) BOUNDED_EQUAL (val, end, string_literal)
  ...
  if (VAL_IS ("unix"))
    ...
    err:
      fprintf (stderr,
	        "%s: %s: Invalid restriction `%s', use `unix' or `windows'.\n" ,
	       "exec_name", com, val);
      return 0;
    }
...
cmd_spec_useragent (const char *com, const char *val, void *place_ignored)
{
  ...
  free_null (opt.useragent);
  opt.useragent = strdup (val);
  return 1;
}

...
cleanup (void)
{
  ...
  free_null (opt.lfilename);
  free_null (opt.dir_prefix);
  free_null (opt.input_filename);
  free_null (opt.output_document);
  ...
  free_null (opt.progress_type);
  free_null (opt.ftp_user);
  free_null (opt.ftp_passwd);
  free_null (opt.ftp_proxy);
  free_null (opt.https_proxy);
  free_null (opt.http_proxy);
  free_vec (opt.no_proxy);
  free_null (opt.useragent);
  free_null (opt.referer);
  free_null (opt.http_user);
  free_null (opt.http_passwd);
  free_vec (opt.user_headers);
# ifdef HAVE_SSL
  free_null (opt.cert_file);
  free_null (opt.private_key);
  free_null (opt.ca_directory);
  free_null (opt.ca_cert);
  free_null (opt.random_file);
  free_null (opt.egd_file);
# endif
  free_null (opt.bind_address);
  free_null (opt.cookies_input);
  free_null (opt.cookies_output);
  free_null (opt.user);
  free_null (opt.passwd);
#endif /* DEBUG_MALLOC */
}
